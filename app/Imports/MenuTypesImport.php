...
class MenuTypesImport implements ToCollection, WithHeadingRow, SkipsEmptyRows, WithBatchInserts, WithChunkReading
{
    ...
    protected function processPreview(Collection $rows): void
    {
        foreach ($rows as $index => $row) {
            ...
            $name = $data['name'] ?? null;
            if (!empty($name)) {
                $existing = MenuType::withoutGlobalScopes()->where('name', $name)->first();
                if ($existing) {
    ...
    protected function processRow(array $row, int $rowNumber): void
    {
        ...
        $name = $data['name'] ?? null;
        $existing = $name ? MenuType::withoutGlobalScopes()->where('name', $name)->first() : null;
    ...
    protected function updateRecord(MenuType $record, array $data, int $rowNumber): void
    {
        try {
            $record->update([
                'name' => $data['name'],
