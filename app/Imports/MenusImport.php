...
class MenusImport implements ToCollection, WithHeadingRow, SkipsEmptyRows, WithBatchInserts, WithChunkReading
{
    ...
    protected function processPreview(Collection $rows): void
    {
        foreach ($rows as $index => $row) {
            ...
            $name = $data['name'] ?? null;
            if (!empty($name)) {
                $existing = Menu::withoutGlobalScopes()->where('name', $name)->first();
                if ($existing) {
    ...
    protected function processRow(array $row, int $rowNumber): void
    {
        ...
        $name = $data['name'] ?? null;
        $existing = $name ? Menu::withoutGlobalScopes()->where('name', $name)->first() : null;
    ...
    protected function createRecord(array $data, int $rowNumber): void
    {
        try {
            Menu::create([
                ...
                'schedule_mode' => $data['schedule_mode'] ?? 'always',
            ]);
    ...
    protected function updateRecord(Menu $record, array $data, int $rowNumber): void
    {
        try {
            $record->update([
                'name' => $data['name'],
