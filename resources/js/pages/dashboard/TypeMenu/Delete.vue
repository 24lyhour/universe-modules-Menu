...
import type { MenuTypeDeleteProps } from '@menu/types';

const props = defineProps<MenuTypeDeleteProps>();
...
const handleSubmit = () => {
    form.delete(`/dashboard/menu-types/${props.menuType.id}`, {
        onSuccess: () => {
            ...
            <div class="flex items-start gap-3 rounded-lg border border-destructive/50 bg-destructive/10 p-4">
                ...
                <div class="space-y-1">
                    <p class="text-sm font-medium text-destructive">
                        You are about to delete this menu type
                    </p>
                    <p class="text-sm text-muted-foreground">
                        <strong>{{ menuType.name }}</strong> will be permanently removed.
                    </p>
                </div>
            </div>
